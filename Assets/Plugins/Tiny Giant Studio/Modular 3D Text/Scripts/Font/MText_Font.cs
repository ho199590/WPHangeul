...
namespace MText
{
    ...
    public class MText_Font : ScriptableObject
    {
        ...
        public float Spacing(char c)
        {
            if (!monoSpaceFont)
            {
                for (int i = 0; i < characters.Count; i++)
                {
                    if (c == characters[i].character)
                    {
                        return ConvertedValue(characters[i].spacing);
                    }
                }
                ...
                return ConvertedValue(monoSpaceSpacing);
            }
            else
            {
                return ConvertedValue(monoSpaceSpacing); //for monospace fonts
            }
        }
        ...
        //    return characterSpacing; //for monospace fonts
        //}
