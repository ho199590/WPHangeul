...
public class LoadingTutorialManager : MonoBehaviour
{
    ...
    //제일 먼저 Scriptable Object에서 가져온 프리팹들을 생성 및 시작위치에 위치시키고 이동을 시작하는 함수 Go()를 호출
    private void Start()
    {
        tutorialObjects.Insert();
        if (tutorialObjects.GetIndex().TryGetValue(nextScene.Split('_')[0], out IntroductionObjects[] objects))
        {
            ...
            for (int i = 0; i < objectsLength; i++)
            {
                ...
                //objects[i].Object.transform.position = objectPosi[i].transform.position;
                objects[i].Object.transform.localScale = new Vector3(1.5f, 1.5f, 1.5f); 
                objectPosi[i] = Instantiate(objects[i].Object, objectPosi[i].transform.position, Quaternion.LookRotation(Vector3.left)); //파괴도 해주기?
    ...
    IEnumerator Move4(int index)
    {
        ...
        while ((currentTime / lerpTime) < 1)
        {
            print("러프체크4");
            currentTime += Time.deltaTime;
            objectPosi[index].transform.position = Vector3.Lerp(pipe.position, new Vector3(pipe.position.x, pipe.position.y + 10, pipe.position.z), currentTime / lerpTime);
            yield return null;
        }
